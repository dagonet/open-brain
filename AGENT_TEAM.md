...
4. PO spawns reviewer-N -> reviews PR -> reviewer-N shuts down
   ...
   (Max 3 fix cycles — then escalate to architect per Rule 8)
...
```
You are {name} on team {team}. Task #{n}.
...
8. **Max 3 fix cycles per task** — then escalate to architect.
...
- **Developer stuck** (>3 attempts at a problem): Message PO, PO may re-spawn architect for targeted guidance.
