...
describe('semanticSearch', () => {
  ...
  it('surfaces lifecycle_status in result rows when present', async () => {
    ...
    const result = JSON.parse(await semanticSearch(mock.client, openai, { query: 'test' }));
  ...
  it('includes score, salience, and project in v2 results', async () => {
    ...
    const result = JSON.parse(await semanticSearch(mock.client, openai, { query: 'hybrid' }));
