...
import { createMockSupabase, createMultiQueryMockSupabase } from "./helpers.js";
...
describe("taskUpdate", () => {
  it("edits title", async () => {
    ...
    const result = JSON.parse(
      await taskUpdate(client, { id: "task-uuid", title: "New title" }),
    );
  ...
  it("status change appends to status_history", async () => {
    const { client } = createMultiQueryMockSupabase([
      ...
      },
      {
        data: {
          ...
          status: "in_progress",
          status_history: [
            { status: "in_progress", at: expect.any(String), note: null },
          ],
          project: null,
          description: null,
          priority: null,
    ...
    const result = JSON.parse(
      await taskUpdate(client, { id: "task-uuid", status: "in_progress" }),
    );
  ...
  it("cancelling a task sets deleted_at", async () => {
    const { client } = createMultiQueryMockSupabase([
      ...
      },
      {
        data: {
          ...
          status: "cancelled",
          status_history: [
            { status: "cancelled", at: expect.any(String), note: null },
          ],
          project: null,
          description: null,
          priority: null,
    ...
    const result = JSON.parse(
      await taskUpdate(client, { id: "task-uuid", status: "cancelled" }),
    );
  ...
  it("returns not found for nonexistent task", async () => {
    ...
    const result = JSON.parse(
      await taskUpdate(client, { id: "nonexistent", title: "New title" }),
    );
  ...
  it("returns error on supabase read failure", async () => {
    ...
    const result = JSON.parse(
      await taskUpdate(client, { id: "error-uuid", title: "New title" }),
    );
