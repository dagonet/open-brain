...
export async function captureThought(params: CaptureThoughtParams): Promise<string> {
  ...
  try {
    ...
    if (data.duplicate_candidate) {
      ...
      result.note = `Near-duplicate detected: ${dc.thought_id} (similarity ${dc.similarity}). Preview: "${dc.raw_text_preview}". Use thoughts_supersede if this replaces it.`;
    }
...
export const definition: ToolDefinition = {
  ...
  description:
    'Capture a thought, decision, insight, or note into Open Brain memory. The system automatically classifies it, extracts people/topics, and generates embeddings.',
  schema: {
    text: z
      .string()
      .describe(
        'The thought to capture. Can be a decision, insight, meeting note, action item, or general note.',
      ),
    metadata: z
      .record(z.unknown())
      .optional()
      .describe("Optional metadata (e.g., { session_context: 'sprint planning' })"),
    project: z
      .string()
      .optional()
      .describe(
        'Project to associate this thought with. Falls back to OPEN_BRAIN_DEFAULT_PROJECT env var if set.',
      ),
  },
