...
# PROJECT DEVIATION (keep-mine): this repo's run-gate.sh is project-owned and
# writes .gate/last-pass.json PRETTY-PRINTED — `"sha": "…"`, space after the
# colon — so the template's compact-only `'"sha":"'` returns EMPTY and every
# merge blocks with a false `artifact sha: none` on a gate that passed.
# Widen BOTH the grep and the sed: widening only the grep leaves a leading
# space in the captured value, which never matches either.
# Upstream has accepted this as a template defect; it ships in v2.4.0, after
# which this deviation can be dropped.
