  ...
  return (
    <div className="rounded-lg border border-[var(--border)] bg-[var(--bg-card)] p-5 hover:border-[var(--accent)]/30 transition-colors">
      <div className="flex items-center gap-3 mb-3">
        <span className={`text-xs px-2.5 py-0.5 rounded-full border ${colorClass}`}>
          {thought.thought_type}
        </span>
        <span className="text-xs text-[var(--text-muted)]">{relativeTime(thought.created_at)}</span>
        {thought.source && (
          <span className="text-xs text-[var(--text-muted)] ml-auto">{thought.source}</span>
        )}
      </div>
