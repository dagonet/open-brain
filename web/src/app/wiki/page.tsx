      ...
      <main className="flex-1 p-6 md:p-8">
        <div className="max-w-4xl mx-auto">
          <header className="mb-6">
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">Wiki pages</h1>
            <p className="text-sm text-[var(--text-secondary)] mt-1">
              Compiled topic pages with provenance back to your source thoughts. Run{' '}
              <code>brain wiki refresh --all</code> to generate or update.
            </p>
          </header>
          ...
          {rows.length === 0 ? (
            <div className="rounded-lg border border-[var(--border)] p-8 text-center text-[var(--text-secondary)]">
              ...
              <p className="text-sm">
                Run <code>brain wiki refresh --all</code> to compile your first pages.
              </p>
            </div>
          ) : (
            <ul className="space-y-2">
              {rows.map((row) => (
                ...
                >
                  <Link href={`/wiki/${encodeURIComponent(row.slug)}`} className="block px-4 py-3">
                    <div className="flex justify-between items-baseline gap-3">
                      <span className="font-medium text-[var(--text-primary)]">{row.slug}</span>
