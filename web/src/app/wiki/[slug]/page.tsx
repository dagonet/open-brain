...
export default async function WikiSlugPage({ params }: PageProps) {
  ...
  const { data: joins } = await supabase
    ...
    .select('thought_id')
    .eq('page_id', wikiPage.id);
  const thoughtIds = ((joins as SourceJoin[] | null) ?? []).map((j) => j.thought_id);
      ...
      <main className="flex-1 p-6 md:p-8">
        <div className="max-w-3xl mx-auto">
          <header className="mb-4">
            ...
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">{wikiPage.slug}</h1>
          ...
          <div className="rounded-md border border-[var(--border)] bg-[var(--bg-soft)] p-3 mb-4 text-sm text-[var(--text-secondary)]">
            ...
            {stale.open_contradictions_count > 0 ? (
              <>
                {' '}
                <Link href="/contradictions?status=open" className="underline text-amber-400">
            ...
            <form action={refreshWikiPage} className="inline">
              ...
              <button type="submit" className="ml-2 underline hover:text-[var(--text-primary)]">
                Refresh now
              </button>
            </form>
          </div>
          ...
          <section className="mt-6">
              ...
              {thoughts.map((t) => (
                <li
                  ...
                  className={`rounded border ${
                    t.deleted_at ? 'border-red-500/40 bg-red-500/5' : 'border-[var(--border)]'
                  } p-3 text-sm`}
                >
                  ...
                  <p className="text-[var(--text-primary)] whitespace-pre-wrap">{t.raw_text}</p>
                </li>
              ))}
              {thoughts.length === 0 ? (
            ...
            {thoughtIds.some((id) => !sourcesById.has(id)) ? (
              <p className="text-xs text-amber-400 mt-2">
                Some cited thoughts could not be loaded — they may have been hard-deleted from the
                database.
              </p>
            ) : null}
          </section>
          ...
          <section className="mt-8 border-t border-[var(--border)] pt-4">
            ...
            <p className="text-sm text-[var(--text-secondary)] mb-2">
              Tell the wiki why this compilation is wrong; the next refresh will see your feedback.
            </p>
