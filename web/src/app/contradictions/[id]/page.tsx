        ...
        <div className="max-w-4xl mx-auto">
          <header className="mb-6">
            <p className="text-sm text-[var(--text-secondary)] mb-1">
              <Link href="/contradictions" className="underline hover:text-[var(--text-primary)]">
                ← All contradictions
              </Link>
            </p>
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">Contradiction</h1>
            <p className="text-xs text-[var(--text-secondary)] mt-1">
              Severity {contra.severity}/5 · confidence {contra.confidence.toFixed(2)} ·{' '}
              {contra.status}
            </p>
          </header>
          ...
          {contra.status === 'open' ? (
            <section className="border-t border-[var(--border)] pt-4">
              <h2 className="text-lg font-semibold text-[var(--text-primary)] mb-2">Resolve</h2>
            ...
            <p className="text-sm text-[var(--text-secondary)]">
              ...
              {contra.resolved_at ? ` (at ${new Date(contra.resolved_at).toLocaleString()})` : ''}.
            </p>
          )}
        </div>
      </main>
