      ...
      <main className="flex-1 p-6 md:p-8">
        <div className="max-w-4xl mx-auto">
          <header className="mb-6">
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">Contradictions</h1>
            <p className="text-sm text-[var(--text-secondary)] mt-1">
              Pairs of your captured thoughts that an audit pass flagged as disagreeing. Run{' '}
              <code>brain audit</code> to scan recent thoughts.
            </p>
          </header>

          <nav className="mb-4 flex gap-2 text-xs">
            {(['open', 'resolved', 'ignored', 'false_positive', 'all'] as const).map((s) => (
              <Link
                key={s}
            ...
            <ul className="space-y-2">
              {contradictions.map((row) => {
                ...
                return (
                  ...
                  >
                    <Link href={`/contradictions/${row.id}`} className="block px-4 py-3">
