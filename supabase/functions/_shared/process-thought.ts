...
export async function processThought(input: ThoughtInput): Promise<ProcessingResult> {
  // 1. Validate input
  ...
  if (metadataResult.status === 'fulfilled') {
    ...
    salience =
      typeof meta.salience === 'number' && meta.salience >= 1 && meta.salience <= 5
        ? meta.salience
        : null;
  } else {
  ...
  const { data, error } = await supabase.from('thoughts').insert(record).select('*').single();
  if (error) {
  ...
  if (entityDescriptions.length > 0) {
    ...
    const { error: entityErr } = await supabase.from('entity_descriptions').insert(rows);
    if (entityErr) {
  ...
  if (embedding) {
    ...
    const threshold =
      Number.isNaN(rawThreshold) || rawThreshold <= 0 || rawThreshold > 1 ? 0.92 : rawThreshold;
    try {
      const { data: nearDups, error: nearDupErr } = await supabase.rpc('find_near_dups', {
      ...
      if (nearDupErr) {
        ...
        duplicateCandidate = {
          thought_id: best.id,
          raw_text_preview:
            best.raw_text.length > 120 ? best.raw_text.slice(0, 120) + '...' : best.raw_text,
